...
#data <- read.table("taxonomy.profiles", header = T, fill = T, sep = "\t",check.names = F)
...
dt2 <- rownames_to_column(aligned_dt,var="name")
...
#save(top_species_result,file="Country.top_species_result.RData")
################
...
#save(dt_processed,file="Country.dt_processed.RData")
####################
