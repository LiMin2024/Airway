...
##############
result <- align_dt_sample(rgi_dt, sample_map2, ID = "Sample")
...
#write.table(mean_abundance_wide,file = "China_abun_sort.txt", sep = "\t", row.names = T, quote = FALSE)

mean_abundance_wide <- read.table("China_abun_sort.txt", header = T, fill = TRUE, sep = "\t",check.names = F)
mean_abundance_wide <- read.table("USA_abun_sort.txt", header = T, fill = TRUE, sep = "\t",check.names = F)
mean_abundance_wide <- read.table("Germany_abun_sort.txt", header = T, fill = TRUE, sep = "\t",check.names = F)
