...
########
data <- read.table("vfdb.profiles", header = T,  sep = "\t",check.names = F)
...
merge <- merge(merge,map,by.x = "name",by.y = "GeneID")
merge <- merge[,c(7,2)]
names(merge)[1] <- "name"
merge <- merge %>%
...
merge <- merge[order(merge$mean, decreasing = TRUE), ]
...
gene.map <- read.table("vfdb.map", header = F, fill = TRUE, sep = "\t",check.names = F)
...
merge3 <- merge3[merge3$V8!= "Unclassified",]
